                    ...
                    <div class="card-body">
                            ...
                            <div class="col-sm-3 col-6">
                                ...
                                @if (Auth::guard('admin')->user()->role == '1' ||
                                    ...
                                    <button type="button" class="btn   btn-sm" id="all_print" target="blank">Package &
                                        Print</button>
                                @endif
                                <a href="{{ route('online_order.export') }}" class="btn btn-success">Online Export</a>
                            </div>
                            <div class="col-sm-3 col-6">
                        ...
                        <form class="" action="" method="GET">
                                    ...
                                    <div class="custom_select">
                                        <select
                                            ...
                                            <option value="partial paid"
                                                @if ($payment_status == 'partial paid') selected @endif>Partial paid
                                            </option>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="table-responsive-sm order-product-show">
                                @include('backend.sales.all_orders.onlineOrder.online_order_products')
                            </div>
                        </form>
    ...
    <script>
        $(document).ready(function() {
            ...
            function fetch_data(page, condition, search, shipping_type, delivery_status, payment_status, dateadd) {
                $.ajax({
                    url: "{{ route('Onlineorder.pagination') }}",
                    data: {
                        page: page,
                        condition: condition,
                        search: search,
            ...
            $(document).on('keyup', '#pro_search', function() {
                ...
                if (search.length > 3) {
                    $.ajax({
                        url: "{{ route('Onlineorder.pro_search') }}",
                        method: "get",
                        data: {
                            search: search,
                            type: 'onlineOrder',
