...
@section('admin')
    ...
    <section class="content-main">
        <div class="content-header">
            <div>
                <h2 class="content-title card-title">All Deleted Order</h2>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <!-- card-header end// -->
                    <div class="card-body">
                        <div class="row mb-2 pack_print" style="justify-content: space-between">
                            <div class="col-sm-3 col-6">
                                <a href="{{ route('order.trash.export') }}" class="btn btn-success">Trash Export</a>
                        ...
                        <form class="" action="" method="GET">
                            ...
                            <div class="table-responsive-sm order-product-show" id="order_print">
                                @include('backend.sales.all_orders.deletedOrder.deletedproducts')
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function fetch_data(page, condition, search, shipping_type, delivery_status, payment_status, dateadd) {
                $.ajax({
                    url: "{{ route('Deleteorder.pagination') }}",
                    data: {
                        page: page,
                        condition: condition,
                        search: search,
            ...
            $(document).on('keyup', '#pro_search', function() {
                ...
                if (search.length > 3) {
                    $.ajax({
                        url: "{{ route('Deleteorder.pro_search') }}",
                        method: "get",
                        data: {
                            search: search,
                            type: 'trashed',
